...
export class TradesService {
  ...
  async buyStock(userId: number, symbol: string, quantity: number) {
    ...
    if (!tick || typeof tick.price !== 'number') {
      throw new NotFoundException(`Pricing information for stock '${symbol}' not available.`);
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (portfolioItem) {
        const oldTotalValue = portfolioItem.avgPrice.mul(portfolioItem.quantity);
      ...
      return { user: updatedUser, portfolioItem: updatedPortfolioItem, transaction };
    });
  }
}
